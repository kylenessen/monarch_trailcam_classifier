...
def plot_deployment_timeseries(df, outliers_dict, output_dir='plots'):
    ...
    for deployment in deployments:
        ...
        for outlier_type, color in outlier_colors.items():
            type_outliers = dept_outliers[dept_outliers['outlier_type'] == outlier_type]
            if not type_outliers.empty:
                type_outliers['datetime'] = pd.to_datetime(type_outliers['timestamp'], format='%Y%m%d%H%M%S')
        ...
        ax1.set_title(f'{deployment} - Full Temperature Range (All Data)', fontsize=14, fontweight='bold')
        ax1.set_ylabel('Temperature (°C)', fontsize=12)
        ax1.grid(True, alpha=0.3)
        ax1.legend()
        
        # Bottom panel: Normal range view (0-35°C)
        normal_data = dept_data[(dept_data['temperature'] >= 0) & (dept_data['temperature'] <= 35)]
        ...
        ax2.set_title(f'{deployment} - Normal Temperature Range (0-35°C)', fontsize=14, fontweight='bold')
...
def save_outliers_summary(outliers_dict, output_file='remaining_outliers.csv'):
    ...
    output_columns = [
        'filename', 'deployment_id', 'timestamp', 'temperature', 
        'confidence', 'extraction_status', 'outlier_type', 'suggested_action'
    ]
    
    outliers_export = all_outliers[output_columns].sort_values(['outlier_type', 'deployment_id', 'timestamp'])
    ...
    for outlier_type, outliers in outliers_dict.items():
        ...
        if len(outliers) > 0:
            if outlier_type == 'high':
                print(f"    Temperature range: {outliers['temperature'].min():.1f}°C - {outliers['temperature'].max():.1f}°C")
...
def main():
    ...
    correction_csvs = ['manual_temperature_entries.csv']  # Add more files as needed
    ...
    if len(outliers_export) > 0:
        print(f"\nNext steps:")
        print(f"  1. Review remaining_outliers.csv ({len(outliers_export)} issues)")
